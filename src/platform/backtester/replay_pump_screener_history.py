...
from src.platform.screeners import scr_pump_binance as pump_module
from src.platform.screeners.scr_pump_binance import PumpConfig, ScrPumpBinance
...
class Reporter:
    ...
    def print_summary(summary: ReplaySummary) -> None:
        ...
        print("=" * 100)
        print("PUMP HISTORICAL REPLAY SUMMARY")
        print("=" * 100)
...
def patched_utc_now(fake_now: datetime):
    original = pump_module._utc_now
    pump_module._utc_now = lambda: fake_now
    try:
        yield
    finally:
        pump_module._utc_now = original
...
def load_yaml_params(path: str) -> Dict[str, Any]:
    ...
    for item in screeners:
        if item.get("name") == "scr_pump_binance":
            return dict(item)
    raise ValueError("scr_pump_binance not found in YAML")


def apply_interval_overrides(params: Dict[str, Any], interval: str) -> Dict[str, Any]:
    return PumpConfig.from_cfg(params).for_interval(interval).as_flat_params()


class PumpHistoricalReplayRunner:
    ...
    def replay(
        ...
        *,
        ...
        print("=" * 120)
        print(f"START PUMP HISTORICAL REPLAY | interval={interval} | points={total_points}")
        print("=" * 120)
        ...
        for i, ts in enumerate(replay_points, start=1):
            ...
            if i == 1 or i % max(1, int(progress_every)) == 0 or i == total_points:
                print(
                    f"[{i}/{total_points}] ts={ts.isoformat()} "
                    f"raw={len(raw_events)} deduped={len(deduped_events)}"
                )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="True historical replay for pump screener: regenerate signals from history, then backtest.")
...
def main() -> None:
    ...
    runner = PumpHistoricalReplayRunner(
        dsn=args.dsn,
