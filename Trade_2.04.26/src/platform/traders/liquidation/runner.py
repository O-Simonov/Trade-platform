...
class TradeLiquidation(
    ...
    def _sync_ledger_open_position_from_exchange(self, *, pos_uid: str, sym: str, side: str, ex_qty: float, ex_entry: float, ex_mark_val: float = 0.0, reason: str = "reconcile_entry_sync") -> bool:
        ...
        Keeps working entry_price/avg_price/value fields aligned with Binance positionRisk so
        recovery/trailing/reconcile operate on the same basis after restarts and partial state rebuilds.
        """
        try:
            pos_uid = str(pos_uid or "").strip()
            if not pos_uid:
                return False
            qty_f = abs(float(ex_qty or 0.0))
            entry_f = abs(float(ex_entry or 0.0))
            if qty_f <= 0.0 or entry_f <= 0.0:
                return False
            val_d = _dec(qty_f * entry_f).quantize(Decimal("0.00000001"))
            meta = {
                "reconcile": {
                ...
                "sync_from_exchange": {
                    "reason": str(reason or "reconcile_entry_sync"),
                    "symbol": str(sym or ""),
                    "side": str(side or ""),
                    "qty": float(qty_f),
                    "entry_price": float(entry_f),
                    "position_value_usdt": float(val_d),
            ...
            if self._pl_has_raw_meta:
                self.store.execute(
                    ...
                    {
                        "q": float(qty_f),
                        "entry": _dec(entry_f).quantize(Decimal("0.00000001")),
                        "avg": _dec(entry_f).quantize(Decimal("0.00000001")),
                        "v": val_d,
            ...
            log.exception(
                "[trade_liquidation][RECONCILE] failed to sync ledger entry from exchange pos_uid=%s (%s %s)",
                str(pos_uid or ""),
                str(sym or ""),
                str(side or ""),
            )
    ...
    def _reconcile_ledger_vs_exchange(self) -> Dict[str, int]:
        ...
        # Ledger position_value_usdt is treated as entry-based notional, not mark-to-market.
        # Keep value auto-adjust OFF by default unless config explicitly enables it.
        auto_adjust_value = bool(getattr(self.p, "reconcile_auto_adjust_ledger_value_usdt", False))
        # Refresh-by-market is dangerous for entry-based ledger value, so keep it OFF by default.
        auto_refresh_value = bool(getattr(self.p, "reconcile_refresh_position_value_usdt", False))
        auto_sync_entry = bool(getattr(self.p, "reconcile_auto_sync_entry_price", True))
        entry_tol_pct = float(getattr(self.p, "reconcile_entry_price_tolerance_pct", 0.001) or 0.001)
        ...
        ex_entry_price: Dict[Tuple[str, str], float] = {}
        ex_mark_usdt: Dict[Tuple[str, str], float] = {}
        for it in rows:
            ...
            if ps in {"LONG", "SHORT"}:
                ...
                side = "LONG" if amt > 0 else "SHORT"
            key = (sym, side)
            ex_qty_abs = abs(float(amt))
            ex_pos[key] = ex_qty_abs
            entry_px = abs(_safe_float(it.get('entryPrice') or it.get('avgEntryPrice') or it.get('avgPrice'), 0.0))
            ex_entry_price[key] = float(entry_px)
            mark_px = abs(_safe_float(it.get('markPrice'), 0.0))
            ex_mark_usdt[key] = abs(ex_qty_abs * float(mark_px)) if mark_px > 0 else 0.0
            # Reconcile ledger value against entry-based notional to avoid perpetual
            # mark-to-market drift between exchange snapshot and ledger bookkeeping.
            if entry_px > 0:
                ex_usdt[key] = abs(ex_qty_abs * float(entry_px))
            else:
                ex_usdt[key] = float(ex_mark_usdt.get(key, 0.0) or 0.0)
        ...
        for p in led:
            ...
            try:
                ...
                value_mode = "entry_notional"
            ex_entry = float(ex_entry_price.get((sym, side), 0.0) or 0.0)
            ex_mark_val = float(ex_mark_usdt.get((sym, side), 0.0) or 0.0)
            ...
            entry_diff_abs = abs(ex_entry - led_entry) if (ex_entry > 0 and led_entry > 0) else 0.0
            entry_diff_rel = (entry_diff_abs / ex_entry) if (ex_entry > 0 and led_entry > 0) else 0.0
            led_expected_entry_val = abs(float(led_qty)) * float(ex_entry) if (led_qty > 0 and ex_entry > 0) else 0.0
            stale_value_only = bool(led_expected_entry_val > 0 and abs(led_expected_entry_val - led_val) > usdt_tol and abs(ex_qty - led_qty) <= tol)
            try:
            ...
            if (not qty_close) or usdt_diff > usdt_tol or (auto_sync_entry and ex_entry > 0 and (led_entry <= 0 or entry_diff_rel > entry_tol_pct)):
                if not qty_close:
                    qty_mismatch += 1
                ...
                if usdt_diff > usdt_tol and bool(getattr(self.p, "reconcile_log_diffs", True)):
                    log.warning(
                        "[trade_liquidation][RECONCILE] value mismatch %s %s: ledger_usdt=%.8f exchange_usdt=%.8f diff=%.8f mode=%s ledger_entry=%.8f ledger_avg=%.8f exchange_entry=%.8f exchange_mark_usdt=%.8f (pos_uid=%s)",
                        sym,
                        side,
                        float(led_val),
                        float(ex_val),
                ...
                if stale_value_only and bool(getattr(self.p, "reconcile_log_diffs", True)):
                    log.warning(
                        "[trade_liquidation][RECONCILE] stale value-only mismatch %s %s: ledger_qty=%.8f exchange_qty=%.8f ledger_usdt=%.8f expected_usdt=%.8f exchange_entry=%.8f (pos_uid=%s)",
                        sym,
                        side,
                        float(led_qty),
                        float(ex_qty),
                        float(led_val),
                        float(led_expected_entry_val),
                        float(ex_entry),
                        str(p.get("pos_uid") or ""),
                    )
                if (auto_sync_entry and ex_entry > 0 and (led_entry <= 0 or entry_diff_rel > entry_tol_pct) and bool(getattr(self.p, "reconcile_log_diffs", True))):
                    log.warning(
                        "[trade_liquidation][RECONCILE] entry mismatch %s %s: ledger_entry=%.8f ledger_avg=%.8f exchange_entry=%.8f rel_diff=%.6f qty=%.8f (pos_uid=%s)",
                        sym,
                        side,
                        float(led_entry),
                        float(led_avg),
                        float(ex_entry),
                        float(entry_diff_rel),
                        float(ex_qty),
                        str(p.get("pos_uid") or ""),
                    )
                ...
                if (auto_adjust_qty or auto_adjust_value or auto_refresh_value or auto_sync_entry) and ex_qty > 0:
                    try:
                        pos_uid = str(p.get("pos_uid") or "")
                        ...
                        do_sync_entry = bool(auto_sync_entry) and ex_entry > 0 and (led_entry <= 0 or entry_diff_rel > entry_tol_pct)
                        do_val = False
                        if ex_val > 0:
                        ...
                        if stale_value_only:
                            do_val = True
                            ex_val = float(led_expected_entry_val or ex_val)

                        if not do_qty and not do_val and not do_sync_entry:
                            continue

                        old_led_qty = float(led_qty)
                        old_led_val = float(led_val)
                        old_led_entry = float(led_entry)
                        old_led_avg = float(led_avg)
                        ...
                        if do_sync_entry:
                            self._sync_ledger_open_position_from_exchange(
                                ...
                                side=side,
                                ex_qty=float(ex_qty),
                                ex_entry=float(ex_entry),
                                ex_mark_val=float(ex_mark_val),
                                reason="reconcile_entry_sync",
                            )
                            led_qty = float(ex_qty)
                            led_val = float(ex_qty * ex_entry)
                            led_entry = float(ex_entry)
                            led_avg = float(ex_entry)
                        ...
                        if self._pl_has_raw_meta:
                            meta = {
                                "reconcile": {
                                    ...
                                    "exchange_entry_price": float(ex_entry),
                                    "exchange_mark_usdt": float(ex_mark_val),
                                },
                                "reconcile_adjust": {
                                    "old_qty": float(old_led_qty),
                                    "new_qty": float(new_qty_f),
                                    "old_usdt": float(old_led_val),
                                    "new_usdt": float(new_val_d),
                                    "ts": _utc_now().isoformat(),
                                    "follow_ts": _utc_now().isoformat(),
                                    "mode": ("entry" if do_sync_entry else "") + ("+qty" if do_qty else "") + ("+val" if do_val else ""),
                                }
                            }
                            self.store.execute(
                                """
                        ...
                        if bool(getattr(self.p, "reconcile_log_diffs", True)):
                            log.info(
                                "[trade_liquidation][RECONCILE] adjusted ledger %s %s old_qty=%.8f new_qty=%.8f old_usdt=%.8f new_usdt=%.8f old_entry=%.8f new_entry=%.8f (pos_uid=%s)",
                                sym,
                                side,
                                float(old_led_qty),
                                float(new_qty_f),
                                float(old_led_val),
                                float(new_val_d),
                                float(old_led_entry if old_led_entry > 0 else (p.get("entry_price") or 0.0)),
                                float(ex_entry if do_sync_entry and ex_entry > 0 else (old_led_entry if old_led_entry > 0 else (p.get("entry_price") or 0.0))),
                                pos_uid,
                            )
                    except Exception:
                        log.exception(
                            "[trade_liquidation][RECONCILE] failed to auto-adjust ledger qty/value/entry (pos_uid=%s)",
                            str(p.get("pos_uid") or ""),
                        )
    ...
    def _auto_recovery_brackets(self) -> int:
        ...
        def _rec_skip(reason: str, *, sym: str = "", side: str = "", pos_uid: str = "", qty_use: float = 0.0, extra: Optional[Dict[str, Any]] = None) -> None:
            try:
                payload = {
                    "sym": str(sym or ""),
                    "side": str(side or ""),
                ...
                for k, v in payload.items():
                    if v in ("", None):
                        continue
                    parts.append(f"{k}={v}")
                log.info("[trade_liquidation][RECOVERY][DECISION] skip reason=%s %s", str(reason), " ".join(parts))
            except Exception:
                pass
        ...
        if bool(getattr(self.p, "recovery_log_snapshot_summary", True)):
            try:
                log.info(
                    "[trade_liquidation][RECOVERY][SNAPSHOT] ledger_open=%d live_rows=%d open_orders=%d open_algo=%d position_risk=%d hedge_managed=%d two_sided=%d max_symbols=%d max_actions=%d",
                    len(led),
                    len(led_live_for_hedge),
                    len(open_rows),
                    len(open_algo_rows),
                    len(pr_rows),
                    len(hedge_managed_sides),
                    len(two_sided_symbols),
                    int(max_symbols),
                    int(max_actions),
                )
            except Exception:
        ...
        if bool(getattr(self.p, "recovery_log_snapshot_summary", True)):
            try:
                live_open_pairs = int(len(ex_has_pos))
                ledger_open_pairs = int(len(led))
                if live_open_pairs != ledger_open_pairs:
                    log.warning(
                        "[trade_liquidation][RECOVERY][SNAPSHOT_DIFF] ledger_open=%d exchange_open=%d diff=%d managed_limit=%d",
                        ledger_open_pairs,
                        live_open_pairs,
                        int(live_open_pairs - ledger_open_pairs),
                        int(max_symbols),
                    )
            except Exception:
                pass
        ...
        for p in led:
            ...
            sym = sym_map.get(symbol_id)
            if not sym:
                _rec_skip("unknown_symbol", pos_uid=str(p.get("pos_uid") or ""), extra={"symbol_id": symbol_id})
                continue
            side = str(p.get("side") or "").upper().strip()
            if side not in {"LONG", "SHORT"}:
                _rec_skip("bad_side", sym=str(sym), side=str(side), pos_uid=str(p.get("pos_uid") or ""))
                continue

            pos_uid = str(p.get("pos_uid") or "").strip()
            if not pos_uid:
                _rec_skip("missing_pos_uid", sym=str(sym), side=str(side))
                continue
            raw_meta = p.get("raw_meta") or {}
            if isinstance(raw_meta, str):
                try:
            ...
            if (sym, side) not in ex_has_pos:
                _rec_skip("no_exchange_position", sym=sym, side=side, pos_uid=pos_uid, qty_use=qty_use, extra={"qty_led": float(qty_led), "qty_ex": float(qty_ex), "source": src_kind})
                continue
            ...
            try:
                ...
                # IMPORTANT:
                # Do NOT suppress generic main _TRL recovery just because the opposite hedge side
                # is open on exchange. In hedge mode we still need:
                #   - normal _TRL for the main side
                #   - dedicated _HEDGE_TRL for the hedge side
                # The only side that must be muted here is the hedge-managed side itself
                # (already filtered above by hedge_managed_sides).
                #
                        ...
                        if bool(sync_after_recovery):
                            try:
                                self._sync_ledger_open_position_from_exchange(
                                    ...
                                    sym=sym,
                                    side=side,
                                    ex_qty=float(ex_qty_map.get((sym, side), qty_use) or qty_use),
                                    ex_entry=float(ex_entry_map.get((sym, side), entry) or entry),
                                    ex_mark_val=float(ex_mark_map.get((sym, side), 0.0) or 0.0),
                                    reason="recovery_trailing_sync",
                                )
                            except Exception:
