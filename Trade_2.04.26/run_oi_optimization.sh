...
# ============================================================
# OI screener optimization runner (Linux/macOS)
# ============================================================
# Usage:
#   1) Edit BACKTEST_DSN and BACKTEST_POOL_DSN below
#      or export them before launch.
#   2) chmod +x run_oi_optimization.sh
#   3) ./run_oi_optimization.sh quick-5m
#      ./run_oi_optimization.sh balanced-15m
#      ./run_oi_optimization.sh balanced-1h
#      ./run_oi_optimization.sh narrow-15m
#      ./run_oi_optimization.sh all-balanced
# ============================================================

: "${BACKTEST_DSN:=postgresql+psycopg2://USER:PASSWORD@HOST:5432/DBNAME}"
: "${BACKTEST_POOL_DSN:=dbname=DBNAME user=USER password=PASSWORD host=HOST port=5432}"
export BACKTEST_DSN
export BACKTEST_POOL_DSN

YAML_PATH="config/screener_oi.yaml"
EXCHANGE_ID="1"
OUT_DIR="artifacts/opt"
mkdir -p "$OUT_DIR"
...
case "${1:-}" in
  quick-5m) run_quick_5m ;;
  balanced-5m) run_balanced_5m ;;
  balanced-15m) run_balanced_15m ;;
  balanced-1h) run_balanced_1h ;;
  narrow-15m) run_narrow_15m ;;
  all-balanced)
    run_balanced_5m
    run_balanced_15m
    run_balanced_1h
    ;;
  *)
    echo "Usage: $0 {quick-5m|balanced-5m|balanced-15m|balanced-1h|narrow-15m|all-balanced}"
    exit 1
    ;;
esac
