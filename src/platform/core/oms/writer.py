...
class OmsWriter:
    ...
    Writer = ТОЛЬКО persistence слой.

    ✔ append immutable order_events
    ✔ upsert trades
    ✔ upsert order snapshot (из OrderAggregate)
    ...
    # ------------------------------------------------------------------
    # ORDER EVENTS (immutable log) — STEP G.1
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # ORDER SNAPSHOT (derived state) — STEP G.2
    # ------------------------------------------------------------------
    def upsert_order_snapshot(self, agg) -> None:
        """
        Persist current OrderAggregate snapshot.

        Aggregate уже:
          - проверил порядок событий
          - применил FSM
          - знает финальный статус
        """
        try:
            row = {
