...
_round_to_tick = _round_price_to_tick

class TradeLiquidationRecoveryMixin:
                        ...
                        if sl_mode in {"trailing_stop_market", "trailing", "tsm"} and bool(getattr(self.p, "trailing_enabled", True)):
                            ...
                            def _place_sl2(i: int):
                                ...
                                try:
                                    ...
                                    stop_tick = float(tick) if tick else 0.0
                                    ...
                                    if mark_sl > 0:
                                        if close_side == "SELL":
                                            ...
                                            if stop_tick > 0:
                                                px = px - stop_tick
                                                px = _round_price_to_tick(px, stop_tick, mode="down")
                                        else:
                                            # short SL: BUY stop must be ABOVE mark
                                            px = max(px, mark_sl * (1.0 + eff_buf / 100.0))
                                            if stop_tick > 0:
                                                px = px + stop_tick
                                                px = _round_price_to_tick(px, stop_tick, mode="up")
                        ...
                        def _place_tp2(i: int):
                            ...
                            try:
                                ...
                                if i > 0 and tick:
                                    ...
                                    if close_side == "SELL":
                                        px = px + float(tick) * i
                                        px = _round_price_to_tick(px, float(tick), mode="up")
                                    else:
                                        px = px - float(tick) * i
                                        px = _round_price_to_tick(px, float(tick), mode="down")
                ...
                try:
                    ...
                    if trl_ao is not None:
                        ...
                        if close_side == "SELL":
                            ...
                            desired_act = _tl_safe_trigger_price(symbol=sym, close_side="SELL", desired=desired_act, mark=mark, tick=tick, buffer_pct=float(getattr(self.p, "trailing_activation_buffer_pct", 0.15) or 0.15))
                        else:
                            desired_act = ref_price * (1.0 - activation / 100.0)
                            desired_act = _tl_safe_trigger_price(symbol=sym, close_side="BUY", desired=desired_act, mark=mark, tick=tick, buffer_pct=float(getattr(self.p, "trailing_activation_buffer_pct", 0.15) or 0.15))
                        ...
                        tol = max(
                    float(tick or 0.0) * float(PRICE_EPS_TICKS),
                    abs(float(desired_act)) * (float(AVG_EPS_PCT) / 100.0),
                    1e-12,
                )
                        if cur_act_f is None or abs(cur_act_f - float(desired_act)) > tol:
                            log.info(
                                f"[trade_liquidation][auto-recovery] stale TRL {sym} {side}: "
        ...
        *,
        ...
        try:
            ...
            for r in pr if isinstance(pr, list) else []:
                ...
                if ps and ps in {"LONG", "SHORT"} and ps != str(side or "").upper():
                    continue
                live_entry = Decimal(str(r.get("entryPrice") or r.get("avgEntryPrice") or r.get("avgPrice") or 0))
                if live_entry > 0:
                    entry_price = live_entry
                    entry_source = "live_entry"
                break
        except Exception:
            pass

        # Min distance using the same compatibility helper as LIVE AVG placement.
        min_pct = Decimal(str(self._cfg_averaging_min_level_distance_pct() or 0))
        if min_pct < 0:
            min_pct = Decimal("0")
        ...
        log.info("[AVG] %s %s ADD1 qty=%.8f source=%s pos_qty=%.8f recovery=1 ref=%.8f ref_source=%s min_pct=%.4f", sym_u, str(side_u).upper(), float(add_qty), str(qty_source), float(pos_qty), float(entry_price), str(entry_source), float(min_pct))
